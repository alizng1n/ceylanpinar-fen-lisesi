    ...
    <style>
        ...
        * {
        ...
        @keyframes avatarPulse {
            0%, 100% { box-shadow: 0 2px 8px rgba(99,102,241,0.3), 0 0 0 0 rgba(99,102,241,0.0); }
            50%       { box-shadow: 0 2px 8px rgba(99,102,241,0.3), 0 0 0 6px rgba(99,102,241,0.18); }
        }
        ...
        @media print {
            ...
            .school-doc-paper.a6-paper h2 {
                font-size: 0.72rem !important;
            }
            .school-doc-paper.a6-paper div,
            .school-doc-paper.a6-paper span,
            .school-doc-paper.a6-paper strong,
            .school-doc-paper.a6-paper td,
            .school-doc-paper.a6-paper th {
                font-size: 0.70rem !important;
            }
            .school-doc-paper.a6-paper .school-doc-body {
                font-size: 0.70rem !important;
                line-height: 1.35 !important;
                margin-top: 0.25rem !important;
                margin-bottom: 0.25rem !important;
            }
            .school-doc-paper.a6-paper .school-doc-footer {
                margin-top: 0.25rem !important;
            }
...
<body class="portal-mode">
    ...
    <header>
        ...
        <div class="header-actions">
            ...
            <span id="db-status-dot" style="width:9px; height:9px; border-radius:50%; background:#64748b; display:inline-block; transition: all 0.3s ease; flex-shrink:0;" title="Veritabanı: Bağlanıyor..."></span>
    ...
    <div id="student-import-modal" class="modal-backdrop" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.55); z-index:9999; align-items:center; justify-content:center; padding:1rem;">
        <div style="background:var(--bg-card); border-radius:var(--radius-lg); padding:2rem; width:100%; max-width:640px; box-shadow:0 20px 60px rgba(0,0,0,0.3); border:1px solid var(--border-color); color:var(--text-primary); position:relative; max-height:85vh; display:flex; flex-direction:column; box-sizing:border-box;">
            ...
            <div style="font-size:0.82rem; color:var(--text-secondary); margin-bottom:1.5rem; line-height:1.5; background:var(--bg-page); border-radius:var(--radius); padding:1rem; border:1px solid var(--border-color);">
                <strong style="color:var(--text-primary); display:block; margin-bottom:0.4rem;">⚠️ Şablon Kuralları:</strong>
                Yükleyeceğiniz Excel dosyasının ilk satırı başlık olmalı ve sırasıyla şu sütunları içermelidir: <br>
                <code style="background:var(--bg-card); padding:0.15rem 0.35rem; border-radius:4px; font-weight:bold; color:var(--accent-color);">NO</code> | 
                <code style="background:var(--bg-card); padding:0.15rem 0.35rem; border-radius:4px; font-weight:bold; color:var(--accent-color);">ADI</code> | 
                <code style="background:var(--bg-card); padding:0.15rem 0.35rem; border-radius:4px; font-weight:bold; color:var(--accent-color);">SOYADI</code> | 
                <code style="background:var(--bg-card); padding:0.15rem 0.35rem; border-radius:4px; font-weight:bold; color:var(--accent-color);">SINIF/ŞUBE</code> (örn: 9/A, 10/B)
                <button onclick="downloadStudentTemplate()" style="display:inline-flex; align-items:center; gap:0.25rem; border:none; background:none; color:#10b981; cursor:pointer; font-weight:600; font-size:0.82rem; padding:0; margin-left:0.5rem; text-decoration:underline;"><i data-lucide="download" size="12"></i> Örnek Şablonu İndir</button>
            </div>

            <!-- Sürükle Bırak / Dosya Seçme Alanı -->
            <div id="student-excel-dropzone" onclick="document.getElementById('student-excel-file-input').click()" style="border:2px dashed var(--border-color); border-radius:var(--radius-lg); padding:2rem; text-align:center; cursor:pointer; transition:all 0.2s; background:var(--bg-page); margin-bottom:1.5rem;">
                <input type="file" id="student-excel-file-input" accept=".xlsx,.xlsm,.xls" style="display:none;" onchange="handleStudentExcelImport(event)">
                <i data-lucide="upload-cloud" style="width:36px; height:36px; color:#10b981; margin-bottom:0.5rem;"></i>
                <p style="margin:0; font-size:0.85rem; font-weight:600; color:var(--text-primary);">Excel Dosyası Seç veya Buraya Sürükle</p>
            ...
            <div id="student-import-preview-section" style="display:none; flex:1; overflow-y:auto; margin-bottom:1.5rem; max-height:220px; border:1px solid var(--border-color); border-radius:var(--radius); box-sizing:border-box;">
                <table style="width:100%; border-collapse:collapse; font-size:0.8rem; text-align:left;">
                    <thead style="background:var(--bg-page); position:sticky; top:0; z-index:2; border-bottom:1px solid var(--border-color);">
                        <tr>
                            <th style="padding:0.5rem; font-weight:600;">No</th>
                            <th style="padding:0.5rem; font-weight:600;">Adı</th>
                            <th style="padding:0.5rem; font-weight:600;">Soyadı</th>
                            <th style="padding:0.5rem; font-weight:600;">Sınıf/Şube</th>
                        </tr>
                    </thead>
                    <tbody id="student-import-preview-body">
                        <!-- JS ile doldurulacak -->
                    </tbody>
                </table>
            </div>
            ...
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div id="student-import-status" style="font-size:0.8rem; color:var(--text-secondary);">Dosya bekleniyor...</div>
                <div style="display:flex; gap:0.75rem;">
                    <button onclick="closeStudentImportModal()" style="padding:0.6rem 1.25rem; border-radius:var(--radius-lg); border:1px solid var(--border-color); background:var(--bg-card); color:var(--text-primary); cursor:pointer; font-weight:600; font-size:0.875rem;">Kapat</button>
                    <button id="btn-start-student-import" onclick="startStudentImport()" disabled style="padding:0.6rem 1.25rem; border-radius:var(--radius-lg); background:var(--accent-gradient); color:#fff; border:none; cursor:pointer; font-weight:600; font-size:0.875rem; display:inline-flex; align-items:center; gap:0.4rem; opacity:0.5;">
                        <i data-lucide="check-circle" size="16"></i> Aktarımı Başlat
                    </button>
                </div>
            </div>
    ...
    <!-- Sınav Derece Listesi (Leaderboard) Modalı -->
    <div id="exam-leaderboard-modal" class="modal-backdrop" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.55); z-index:9999; align-items:center; justify-content:center; padding:1rem;">
        <div style="background:var(--bg-card); border-radius:var(--radius-lg); padding:2rem; width:100%; max-width:800px; box-shadow:0 20px 60px rgba(0,0,0,0.3); border:1px solid var(--border-color); color:var(--text-primary); position:relative; max-height:85vh; display:flex; flex-direction:column; box-sizing:border-box;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.25rem;">
                <div>
                    <h3 id="exam-leaderboard-title" style="margin:0; color:var(--text-primary); font-size:1.25rem; font-weight:700;">Deneme Sınavı Sıralama Listesi</h3>
                    <p id="exam-leaderboard-subtitle" style="margin:0.25rem 0 0 0; font-size:0.8rem; color:var(--text-secondary);">Sınav katılımcı listesi derecelerine göre sıralanmıştır.</p>
                </div>
                <button onclick="closeExamLeaderboardModal()" style="background:none; border:none; cursor:pointer; color:var(--text-secondary); padding:0.25rem; display:inline-flex; align-items:center; justify-content:center; transition:color 0.2s;" onmouseover="this.style.color='var(--text-primary)'" onmouseout="this.style.color='var(--text-secondary)'">
                    <i data-lucide="x" style="width:24px; height:24px;"></i>
                </button>
            </div>
            
            <div style="flex:1; overflow-y:auto; margin-bottom:1.5rem; border:1px solid var(--border-color); border-radius:var(--radius); box-sizing:border-box; background:var(--bg-page);">
                <table style="width:100%; border-collapse:collapse; font-size:0.85rem; text-align:left;">
                    <thead style="background:var(--bg-card); position:sticky; top:0; z-index:2; border-bottom:2px solid var(--border-color); box-shadow:0 2px 5px rgba(0,0,0,0.02);">
                        <tr>
                            <th style="padding:0.75rem; font-weight:700; width:60px; text-align:center;">Sıra</th>
                            <th style="padding:0.75rem; font-weight:700; width:80px; text-align:center;">Okul No</th>
                            <th style="padding:0.75rem; font-weight:700;">Adı Soyadı</th>
                            <th style="padding:0.75rem; font-weight:700; width:100px; text-align:center;">Sınıf/Şube</th>
                            <th style="padding:0.75rem; font-weight:700; width:90px; text-align:center;">Toplam Net</th>
                            <th style="padding:0.75rem; font-weight:700; width:100px; text-align:center;">Puan</th>
                        </tr>
                    </thead>
                    <tbody id="exam-leaderboard-body">
                        <!-- JS ile doldurulacak -->
                    </tbody>
                </table>
            </div>
            ...
            <div style="display:flex; justify-content:flex-end;">
                <button onclick="closeExamLeaderboardModal()" style="padding:0.6rem 1.5rem; border-radius:var(--radius-lg); border:1px solid var(--border-color); background:var(--bg-card); color:var(--text-primary); cursor:pointer; font-weight:600; font-size:0.875rem; transition:all 0.2s;" onmouseover="this.style.background='var(--bg-page)'" onmouseout="this.style.background='var(--bg-card)'">Kapat</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            document.addEventListener('keydown', (event) => {
                if (event.key === 'Escape') {
                    ...
                    const leaderboardModal = document.getElementById('exam-leaderboard-modal');
                    if (leaderboardModal && leaderboardModal.style.display !== 'none') {
                        closeExamLeaderboardModal();
                    }
                }
            });
        });
        ...
        function updateDbStatusBadge(isCloud) {
            ...
            if (isCloud) {
                dot.title = 'Bulut Bağlantısı Aktif (Supabase)';
                dot.style.background = '#10b981';
                dot.style.boxShadow = '0 0 10px 2px rgba(16,185,129,0.55)';
            } else {
                dot.title = 'Bağlantı Yok (Çevrimdışı)';
                ...
                docHTML = `
                    <div class="school-doc-paper a6-paper">
                        ...
                        <div class="school-doc-footer" style="display:flex; justify-content:space-between; margin-top:0; font-size:0.8rem;">
                            <div style="text-align:left; font-size:0.75rem; line-height:1.3; align-self:flex-end; white-space:nowrap;">
...
function showSchoolManagementView() {
        ...
        function setupDragAndDrop() {
            ...
            const studentDropzone = document.getElementById('student-excel-dropzone');
            if(studentDropzone) {
                ['dragenter', 'dragover'].forEach(eventName => {
                    studentDropzone.addEventListener(eventName, (e) => {
                        e.preventDefault();
        ...
        function clearStudentExcelFile() {
            ...
            const statusDiv = document.getElementById('student-import-status');
        ...
        function parseStudentExcel(file) {
            ...
            reader.onload = function(e) {
                try {
                    ...
                    const adIdx = headers.indexOf('ADI');
                    const soyadIdx = headers.indexOf('SOYADI');
                    ...
                    for (let r = 1; r < rows.length; r++) {
                        ...
                        const rawNo = row[noIdx];
                        const rawAd = row[adIdx];
                        const rawSoyad = row[soyadIdx];
                        const rawSinif = row[sinifIdx];

                        if (!rawAd || String(rawAd).trim() === "" || !rawSoyad || String(rawSoyad).trim() === "") continue;

                        const no = parseInt(rawNo, 10);
                        const ad = String(rawAd).trim().toUpperCase();
                        const soyad = String(rawSoyad).trim().toUpperCase();
                        const snf = String(rawSinif || '').trim().toUpperCase();

                        if (isNaN(no)) continue;

                        parsedStudentsToImport.push({ no, ad, soyad, snf });
                        previewHtml += `
                            <tr style="border-bottom:1px solid var(--border-color);">
                                <td style="padding:0.5rem; color:var(--text-secondary);">${no}</td>
                                <td style="padding:0.5rem; font-weight:600; color:var(--text-primary);">${ad}</td>
                                <td style="padding:0.5rem; font-weight:600; color:var(--text-primary);">${soyad}</td>
                                <td style="padding:0.5rem; color:var(--text-secondary);">${snf}</td>
                            </tr>
                        `;
                    }
        ...
        function downloadStudentTemplate() {
            try {
                const headers = [["NO", "ADI", "SOYADI", "SINIF/ŞUBE"]];
                const sampleRows = [
                    [1, "ALİ RIZA", "BOZ", "9/A"],
                    [4, "ELİF", "TEKE", "10/B"],
                    [6, "SÜMEYYE RANA", "ASLAN", "11/A"]
                ];
                ...
                link.download = 'ceylanpinar_ogrenci_sablonu.xlsx';
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                
                showToast("Örnek öğrenci şablonu indirildi.");
            } catch (error) {
        ...
        function downloadExamTemplate() {
            try {
                const link = document.createElement('a');
                link.href = 'ceylanpinar_ornek_sinav_sablonu.xlsx';
                ...
                console.error("Exam template download error:", error);
                showToast("Sınav şablonu indirilirken hata oluştu.", "error");
            }
        }
        ...
        function displayStudentDashboard(studentNo) {
            ...
            let rankCount = 0;
            studentRecords.forEach(r => {
                if (r.siralama && r.siralama > 0) {
                    totalRank += r.siralama;
                    rankCount++;
                }
            });
            ...
            avgRow.innerHTML = `
                ...
                <td class="cell-score" style="text-align:center;">-</td>
                <td class="cell-rank" style="text-align:center; font-weight:bold;">${avgRank}</td>
            `;
        ...
        function displayStudentReport(records) {
            ...
            records.forEach(r => {
                ...
                tr.innerHTML = `
                    ${isAdmin ? `
                    <td class="results-delete-cell" style="text-align:center; vertical-align:middle; width: 50px;">
        ...
        function showExamLeaderboard(examName) {
            const modal = document.getElementById('exam-leaderboard-modal');
            const title = document.getElementById('exam-leaderboard-title');
            const tbody = document.getElementById('exam-leaderboard-body');
            
            if (title) title.innerText = examName;
            if (tbody) {
                tbody.innerHTML = '';
                
                const examRecords = examDatabase.filter(r => r.deneme === examName);
                examRecords.sort((a, b) => {
                    const rankA = a.siralama || 999999;
                    const rankB = b.siralama || 999999;
                    if (rankA !== rankB) return rankA - rankB;
                    
                    const netA = a.subjects && a.subjects.toplam ? a.subjects.toplam.n : 0;
                    const netB = b.subjects && b.subjects.toplam ? b.subjects.toplam.n : 0;
                    return netB - netA;
                });
                
                if (examRecords.length === 0) {
                    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding:2rem; color:var(--text-secondary);">Bu sınava ait kayıt bulunamadı.</td></tr>`;
                } else {
                    examRecords.forEach((r, idx) => {
                        const tr = document.createElement('tr');
                        tr.style.borderBottom = '1px solid var(--border-color)';
                        tr.style.transition = 'background 0.2s';
                        
                        let rankBadgeHtml = r.siralama || (idx + 1);
                        if (r.siralama === 1) {
                            rankBadgeHtml = `<span style="background:#eab308; color:#fff; padding:0.2rem 0.5rem; border-radius:50px; font-size:0.75rem; font-weight:700; display:inline-flex; align-items:center; gap:0.25rem;"><i data-lucide="trophy" style="width:12px; height:12px;"></i> 1</span>`;
                        } else if (r.siralama === 2) {
                            rankBadgeHtml = `<span style="background:#cbd5e1; color:#1e293b; padding:0.2rem 0.5rem; border-radius:50px; font-size:0.75rem; font-weight:700; display:inline-flex; align-items:center; gap:0.25rem;"><i data-lucide="medal" style="width:12px; height:12px;"></i> 2</span>`;
                        } else if (r.siralama === 3) {
                            rankBadgeHtml = `<span style="background:#b45309; color:#fff; padding:0.2rem 0.5rem; border-radius:50px; font-size:0.75rem; font-weight:700; display:inline-flex; align-items:center; gap:0.25rem;"><i data-lucide="award" style="width:12px; height:12px;"></i> 3</span>`;
                        }
                        
                        const netVal = r.subjects && r.subjects.toplam ? r.subjects.toplam.n.toFixed(2) : '0.00';
                        const scoreVal = r.puan > 0 ? r.puan.toFixed(3) : '-';
                        const clsBranch = `${r.class || ''} / ${r.branch || '-'}`;
                        
                        tr.innerHTML = `
                            <td style="padding:0.75rem; text-align:center; vertical-align:middle;">${rankBadgeHtml}</td>
                            <td style="padding:0.75rem; text-align:center; color:var(--text-secondary); vertical-align:middle;">${r.no || '-'}</td>
                            <td style="padding:0.75rem; font-weight:600; color:var(--text-primary); vertical-align:middle;">${r.name}</td>
                            <td style="padding:0.75rem; text-align:center; color:var(--text-secondary); vertical-align:middle;">${clsBranch}</td>
                            <td style="padding:0.75rem; text-align:center; font-weight:700; color:var(--accent-color); vertical-align:middle;">${netVal}</td>
                            <td style="padding:0.75rem; text-align:center; color:var(--text-secondary); vertical-align:middle;">${scoreVal}</td>
                        `;
                        
                        tr.onmouseover = () => { tr.style.background = 'rgba(59,130,246,0.03)'; };
        ...
        function closeExamLeaderboardModal() {
            ...
            let tableHTML = '<p>Tablo bulunamadı.</p>';
            if (tableEl) {
                const tempTable = tableEl.cloneNode(true);
                const delHeader = tempTable.querySelector('#results-delete-header');
                if (delHeader) delHeader.remove();
                tempTable.querySelectorAll('.results-delete-cell').forEach(cell => cell.remove());
                const totalRowTitle = tempTable.querySelector('.total-row td');
                if (totalRowTitle) {
                    totalRowTitle.setAttribute('colspan', '1');
                }
