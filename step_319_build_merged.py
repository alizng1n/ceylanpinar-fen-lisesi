...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Modern Redesigned Footer (Addresses copyright text removal) */
        footer {
            background: var(--bg-card);
            border-top: 1px solid var(--border-color);
            padding: 2.5rem 2rem;
        ...
        body.portal-mode footer .footer-col h4 {
            color: #ffffff !important;
        }

        [data-theme="dark"] body.portal-mode footer .footer-col h4 {
            color: #ffffff !important;
        }

        body.portal-mode footer .footer-col p {
            color: #475569 !important;
        }

        [data-theme="dark"] body.portal-mode footer .footer-col p {
            color: rgba(255, 255, 255, 0.75) !important;
        }
...
<body class="portal-mode">
    ...
    <script>
        ...
        // e-Okul News Scraper/Proxy Parser
        ...
        // Browser Navigation (HTML5 History API Router)
        function initRouter() {
            const currentHash = window.location.hash || '#home';
            const stateName = currentHash.replace('#', '');
            
            // Register initial home state
            window.history.replaceState({ state: stateName }, "", currentHash);
            navigate(stateName, false);

            window.addEventListener('popstate', (event) => {
                const targetState = event.state && event.state.state ? event.state.state : 'home';
        ...
        function navigate(state, push = true) {
            ...
            if ((state === 'teacher-hub' || state === 'teacher-analysis') && sessionStorage.getItem('teacher_logged_in') !== 'true') {
                state = 'home';
                push = false;
